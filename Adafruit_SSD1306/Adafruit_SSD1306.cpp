...
void Adafruit_SSD1306::ssd1306_command1(uint8_t c) {
  ...
  if (wire) { // I2C
    ...
    //sprintf(debug_message, "SPI CMD1=%02x %c", (unsigned int)c,0x0d);  //peter
    //Serial.println(debug_message);   //peter
...
void Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  ...
  if (wire) { // I2C
    ...
    SSD1306_MODE_COMMAND
    while (n--) {
      //sprintf(debug_message, "SPI CMD0=%02x %c", (unsigned int)(*c),0x0d);  //peter
      //Serial.println(debug_message);    //peter
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  if (wire) { // Using I2C
    ...
    //sprintf(debug_message, "SSD1306 begin I2C %c", 0x0d);  //peter
    //Serial.println(debug_message);   //peter
    ...
    //sprintf(debug_message, "SSD1306 begin SPI Width:%d Hight:%d %c",WIDTH, HEIGHT, 0x0d);  //peter
    //Serial.println(debug_message);   //peter
    //sprintf(debug_message, "SSD1306 begin SPI CLK:%d IO:%d DC:%d CS:%d %c",clkPin, mosiPin ,dcPin, csPin, 0x0d);  //peter
    //Serial.println(debug_message);   //peter
    ...
    if (spi) { // Hardware SPI
      ...
      //Serial.println("SSD1306 begin Hardware SPI\n");   //peter
      if (periphBegin)
        spi->begin();
    } else {                    // Soft SPI
      Serial.println("SSD1306 begin Software SPI");   //peter  
  ...
  static const uint8_t PROGMEM init3[] = {SSD1306_MEMORYMODE,         // 0x20                    // Peter disable
                                          //0x00,                     // 0x0 act like ks0108     // Peter disable
                                          0x00,                       // 0x0 act like ks0108     // Peter disable
                                          SSD1306_SEGREMAP | 0x1,       // A1
  ...
  ssd1306_command1((vccstate == SSD1306_EXTERNALVCC) ? 0x22 : 0xF1); // 0xF1
  static const uint8_t PROGMEM init5[] = {
      SSD1306_SETVCOMDETECT,                                        // 0xDB
      0x40,                                                         // 0x40
      SSD1306_DISPLAYALLON_RESUME,                                  // 0xA4
      SSD1306_NORMALDISPLAY,                                        // 0xA6
      //SSD1306_DEACTIVATE_SCROLL,                                  // 0x2E                     //Peter disable 
      SSD1306_DISPLAYON}; // Main screen turn on                    // 0xAF
...
void Adafruit_SSD1306::display(void) {
  ...
  if (wire) { // I2C
    ...
    SSD1306_MODE_DATA     
    while (count--)  {
      //sprintf(debug_message, "SPI DAT=%c %c", (unsigned int)*ptr,0x0d);  //peter
      //Serial.println(debug_message);   //peter  
      SPIwrite(*ptr++);
    }
  }
